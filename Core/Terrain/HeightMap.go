...
func (heightMap *HeightMap) GetHeight(x int, z int) float32 {
	...
	return heightMap.data[z*heightMap.NumColumns+x]
}

func (heightMap *HeightMap) GetHeightScaled(x int, z int) float32 {
	return heightMap.GetHeight(x, z) * heightMap.Scale
}

func (heightMap *HeightMap) GetNormal(x int, z int) GeometryMath.Vector3 {
	heightL := heightMap.GetHeightScaled(x-1, z)
	heightR := heightMap.GetHeightScaled(x+1, z)
	heightD := heightMap.GetHeightScaled(x, z-1)
	heightU := heightMap.GetHeightScaled(x, z+1)
